 ...
 * Provides UI for adding, filtering, importing, and exporting CARs.
 */
export const CarControls = ({ onAddItem, onImport, onExport, onClearData, filters, setFilters, title, tableView, setTableView }) => {
                     ...
                     Papa.parse(fileContent, {
                        ...
                        complete: (results) => {
                            ...
                            data = results.data.map((item, index) => ({
                                id: item.id || `car-${Date.now()}-${index}`,
                                description: item.description || 'Imported CAR',
    ...
    const exportItems = async (format) => {
        ...
        switch (format) {
            ...
            case 'pdf': {
                ...
                autoTable(doc, {
                    startY: 30,
                    head: [['Description', 'Status', 'Assigned To', 'Due Date', 'Corrective Action', 'Notes']],
                    body: items.map(i => [i.description, i.status, i.assignedTo, i.dueDate, i.correctiveAction, i.notes]),
                    styles: { fontSize: 8 },
            ...
            case 'xml':
                const xmlString = jsonToXML(items, 'CorrectiveActionRequests', 'CAR');
    ...
    return html`
        <div className="controls-container manufacturer-controls">
            <div className="main-controls">
                <button type="button" className="control-btn" onClick=${onAddItem}>Add CAR</button>
            ...
            ${showFilters && html`
                <div className="filter-grid">
                    <input type="text" name="description" placeholder="Filter by Description..." value=${filters.description || ''} onInput=${handleFilterChange} className="form-control" aria-label="Filter by Description" />
